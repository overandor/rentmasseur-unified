...
The protocol separates concerns into four independent layers. Execution is only one layer. Everything above it survives migration. Everything below it proves migration occurred.
...
The key insight: **State != Execution**. Execution is temporary. State is durable. Hosts become interchangeable. Authenticated state becomes continuous.

## Execution broker

The execution broker (`execution_broker.py`) dispatches signed capsules to available execution environments based on the capsule's declared requirements. The provider returns a successor capsule and evidence packet. The broker keeps the protocol consistent regardless of where execution occurred.
...
+--------+--------+--------+
|        |        |        |
Local   Colab    SSH     Custom
|        |        |        |
+--------+--------+--------+
    ↓
Successor Capsule + Evidence Packet
    ↓
Verification (protocol unchanged)
```

Providers:
- **local** — execute on the same machine (for testing)
- **colab** — generate dispatch record for Google Colab manual execution
- **ssh** — execute on remote host via SSH/SCP
- **custom** — register your own provider via the SDK

The broker does not change the trust model. It changes the dispatch model. The same 15-check verifier runs regardless of provider.

## Functional repository structure

The repository is organized by function, not by history. Dated experiments live in `archive/`.

```text
runtime/        — execution scripts, broker, bundle demo
transport/       — deploy packages, signed capsules
verification/    — verifier, test suites
security/        — secret scanner, negative evidence
evidence/        — claims, milestones, ledgers, FSM, coverage
sdk/             — broker API entry point
examples/        — Colab notebook, E2B proof
docs/            — runbooks, PRD, instructions
investor/        — SEED_PITCH, investment summary
archive/         — dated runs, old experiments, caches
```

An investor or customer evaluating the project for the first time navigates by function, not by date.

## Maturity ladder

Progress measured by increasing evidence, not increasing claims.

| Level | Milestone | Status |
| ----- | --------- | ------ |
| 0 | Local replay | Done |
| 1 | Cross-platform continuation | Done |
| 2 | Independent cloud continuation | Done (3 hosts: E2B, Codespaces, local) |
| 3 | Multi-hop lineage (E1 -> E2 -> E3 -> E4) | Pending |
| 4 | Independent verifier reproduction (3+ parties) | Pending |
| 5 | Real agent continuation (non-deterministic state) | Pending |
| 6 | Adversarial host evaluation | Pending |
| 7 | Hardware-backed attestation | Pending |
| 8 | Production deployment | Pending |

## Precise demonstrated claim

> HDAR has demonstrated a reproducible proof-of-concept for cryptographically authorized state continuation across heterogeneous execution environments. The current evidence supports cross-platform restoration, deterministic continuation, cryptographic lineage, and independently signed successor artifacts. It does not yet establish resistance to malicious hosts, hardware-backed attestation, or generalized continuation across arbitrary agent frameworks, but it provides a credible foundation for those next stages.

## What to avoid claiming

- "general agent migration solved"
- "hardware-independent consciousness"
- "arbitrary process migration"
- "production-grade secure infrastructure"

Those require substantially broader evidence.
...
Verifier (third-party)
        │  receipt: 15-check verification output
        ▼
Customer Receipt
...
**5. Objectively checkable task result.** The continued task is a 5-stage analysis pipeline (parse → filter → aggregate → classify → report) with a deterministic output hash. The hash is recomputed each run from the input records and verified independently by the semantic correctness check. The task gives the verifier an unambiguous state-continuation predicate: the output hash must match exactly across independent executions.

**6. Independent verification suite.** The verifier reports 15/15 successful checks on three independent hosts: E2B cloud sandbox (Linux 6.1.158+, x86_64), GitHub Codespaces on Azure (Linux 6.8.0-1052-azure, x86_64), and local macOS (arm64). This is externally reproduced evidence across two cloud providers, not merely local self-verification. The verifier runs on Host A after sandbox/codespace shutdown, establishing operational separation. Checks include independent semantic recomputation (Tier 2), Merkle-like stage chain verification, sandbox lifecycle verification, and environment manifest verification with pinned cryptography==44.0.1.
...
Technical truth and commercial truth are different ledgers. Mixing them weakens both.

**Technical ledger** (`technical_ledger.json`): implemented, verified, externally reproduced, production tested. A technical proof should never imply revenue.

**Commercial ledger** (`commercial_ledger.json`): interviewed, pilot agreed, pilot executed, paid, renewed, expanded. Revenue should never imply technical correctness.

**Negative evidence ledger** (`negative_evidence/`): what the system refuses. A protocol is often defined more clearly by what it rejects than by what it accepts. Currently 8 negative tests (replay, tampered manifest, bad signature, path traversal, symlinks, runner hash mismatch, bundle hash mismatch, corrupted workspace).

**Protocol FSM** (`protocol_fsm.json`): the protocol expressed as a finite state machine. No state transition occurs without evidence. States: CREATED -> SIGNED -> PACKAGED -> TRANSFERRED -> AUTHENTICATED -> EXECUTED -> CONTINUED -> SUCCESSOR_CREATED -> VERIFIED -> ACCEPTED -> ARCHIVED. Terminal failure states: REJECTED, FAILED.

**Evidence graph directory structure:**

```text
claims/        — individual claim files (CLM-0001 through CLM-0012)
evidence/      — evidence registry (EVP-0001 through EVP-0020)
milestones/    — executable milestone predicates (M-30, M-60, M-90)
negative_evidence/ — rejection tests (NEG-001 through NEG-008)
```

Every claim links to evidence. Every evidence links to artifacts. Every artifact has a hash. The investor never needs to search through repositories — they follow links.
...
- Deterministic continuation (5-stage analysis pipeline, output hash verified)
...
- Third-party verification program (15 checks, 15/15 on E2B)
...
Completed on three independent hosts across two cloud providers:

1. **E2B cloud sandbox** (Linux 6.1.158+, x86_64) — 15/15 verifier checks passed
2. **GitHub Codespaces on Azure** (Linux 6.8.0-1052-azure, x86_64) — 15/15 verifier checks passed
3. **Local macOS** (arm64) — development testing

All runs produced 15/15 verifier checks including platforms_differ, semantic_correctness, stage_chain, sandbox_terminated, and environment_manifest. The 5-stage analysis pipeline produced an identical output hash (`8708384aa5f7118c...`) on all hosts. The verifier runs on Host A after sandbox/codespace shutdown, establishing operational separation between Host B and Verifier C.

Proof artifacts saved to `e2b-results/` and `codespace-results/`:
- `host_b_report.json` — full Host B execution report with Ed25519 signature
- `host_b_evidence_packet.json` — independently signed evidence packet
- `successor_capsule_epoch_2.tar.gz` — sealed successor capsule with lineage
- `environment_manifest.json` — pinned dependency manifest (cryptography==44.0.1)
- `verifier_output.json` — 15/15 verifier output with version binding and verifier SHA-256
- `proof_packet_manifest.json` — complete proof packet with lifecycle events and separation model
...
## Research milestones (increasing evidence)

### Level 3 -- Multi-hop lineage

```text
A → E1 → B → E2 → C → E3 → D → E4
```

Every transition independently verified. The protocol must preserve lineage across more than two hosts.

### Level 4 -- Independent verifier reproduction

Publish E1, runner, verifier, instructions. Three independent engineers run Host C, restore, continue, generate E2, verify. If all three obtain the same outcome, the protocol becomes substantially more persuasive than a single-team demonstration.

### Level 5 -- Real agent continuation

Instead of deterministic arithmetic, demonstrate preservation of an evolving workflow:

```text
Host A
    ↓ Analyze 10,000-file repository
    ↓ Complete 40%
    ↓ Suspend
    ↓ E1
    ↓ Host B
...
Now the protocol demonstrates preservation of an evolving workflow rather than just deterministic arithmetic.

### Level 6 -- Adversarial host evaluation

Host B attempts to cheat: submit false results, sign incorrect work, claim continuation without execution. The protocol must detect and reject these attempts.

### Level 7 -- Hardware-backed attestation

The verifier gains cryptographic evidence about the execution environment itself (e.g., SGX, SEV-SNP, TDX) rather than relying only on software-generated reports.

## Claim coverage ratio

The strongest long-term metric is not repository size, code volume, or number of demonstrations. It is:

```text
Claim Coverage = Claims with machine-verifiable evidence / Total public claims
```

Current ratio: **1.0** (12/12 claims have evidence). See `claim_coverage.json`.

A ratio approaching 1.0 means nearly every externally stated capability is backed by an independently inspectable artifact. That changes the company's operating model: instead of asking investors to trust interpretations, the organization asks them to inspect evidence.
...
| The task continued correctly | 5-stage pipeline output hash matches | Independently recompute pipeline output hash | Verified (independent — hash matched on E2B) |
...
| A third party can validate the chain | Verifier program and expected checks | Verifier C runs independently | Verified (15/15 on E2B + 15/15 on Codespaces, post-shutdown) |
| The system is commercially useful | Pricing and use-case hypothesis | Paid pilot or signed design partnership | Unverified |
| The system supports repeatable multi-host execution | Three completed independent Host B runs (E2B + Codespaces + local) | Repeat across multiple hosts and environments | Partially verified (3 independent hosts, 2 cloud providers; more needed) |
...
The Host A → Host B → Verifier C demonstration was completed outside the founder-controlled environment on three independent hosts:

1. **E2B cloud sandbox** (Linux 6.1.158+, x86_64) — 15/15 checks passed
2. **GitHub Codespaces on Azure** (Linux 6.8.0-1052-azure, x86_64) — 15/15 checks passed
3. **Local macOS** (arm64) — for development testing

The E2B chain ran in a single command: `E2B_API_KEY="e2b_..." python3 run_e2b_proof.py`. The Codespaces chain used `gh codespace ssh`/`gh codespace cp` for file transfer and remote execution, with the verifier running on Host A after codespace disconnect.

Results:

- [x] At least one genuinely independent Host B (E2B cloud sandbox, Linux x86_64)
...
- [x] Independently reproduced task result (5-stage pipeline, output hash matched)
- [x] Verifier C acceptance (15/15 checks on E2B, 15/15 on Codespaces, post-shutdown)
...
### Day 60: Repeatability across environments -- IN PROGRESS

The protocol has been reproduced on three independent hosts across two cloud providers (E2B, GitHub Codespaces/Azure) and local macOS. The next phase confirms portability with additional environments and negative tests.

Acceptance criteria:

- [x] Three or more independently controlled hosts (E2B, Codespaces, local)
- [x] At least two operating-system or runtime configurations (Linux x86_64, macOS arm64)
- [x] Repeated clean-environment execution
- [x] Documented dependency installation (cryptography==44.0.1 pinned)
- [x] Deterministic manifest behavior (output hash matched on all hosts)
- [x] Machine-readable verifier output (JSON with 17 boolean predicates)
...
- [x] Independent execution environment (E2B cloud sandbox, Linux 6.1.158+ x86_64)
- [x] Independent reproduction (11/11 verifier checks passed)
- [x] Cross-platform verification (macOS arm64 → Linux x86_64)
- [x] Multi-stage pipeline task (parse → filter → aggregate → classify → report)
- [x] Deterministic output hash verification across platforms
...
```
[ Externally reproduced prototype — 8.5/10 — 11/11 checks passed on independent Linux host (E2B cloud sandbox, Linux 6.1.158+, x86_64) ]
```

Independent Host B run completed:
- Host A: macOS (founder Mac, arm64)
- Host B: Linux 6.1.158+ x86_64 (E2B cloud sandbox)
- Verifier C: same E2B sandbox (operationally separate from Host A)
- Result: 11/11 checks passed, including platforms_differ
- Task: 5-stage analysis pipeline, output hash matched across platforms
- Artifacts saved to: `e2b-results/`
