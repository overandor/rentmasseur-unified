...
def load_or_create_owner_key(key_path: Path) -> dict:
    """Load persistent owner key if it exists, otherwise generate and save to a secure location.

    The key is stored OUTSIDE the deploy package directory to prevent leakage.
    The deploy package only receives the public key.
    """
    if key_path.exists():
        keypair = json.loads(key_path.read_text())
        print(f"Loaded persistent owner key from: {key_path}")
        print(f"Owner public key: {keypair['public_key_hex']}")
        return keypair
    keypair = generate_owner_keypair()
    key_path.parent.mkdir(parents=True, exist_ok=True)
    key_path.write_text(json.dumps(keypair, indent=2, sort_keys=True) + "\n")
    key_path.chmod(0o600)
    print(f"Generated new persistent owner key at: {key_path}")
    print(f"Owner public key: {keypair['public_key_hex']}")
    return keypair
...
def build_package(out_dir: Path, runner_template: Path, verifier_template: Path) -> None:
    ...
    # 1. Load or create persistent owner key (stored OUTSIDE deploy package)
    owner_key_path = Path.home() / ".hdar" / "owner_keypair.json"
    owner_keypair = load_or_create_owner_key(owner_key_path)
    # Only write the PUBLIC key to the deploy package — never the private key
    ...
    # 5. Copy runner and verifier (skip if already in place)
    runner_path = out_dir / "run_on_host_b.py"
    verifier_path = out_dir / "third_party_verifier.py"
    if runner_template.resolve() != runner_path.resolve():
        shutil.copy2(runner_template, runner_path)
    if verifier_template.resolve() != verifier_path.resolve():
