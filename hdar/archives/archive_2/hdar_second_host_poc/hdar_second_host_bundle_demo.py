...
def write_host_b_bundle(capsule_tar: dict, bundle_path: Path) -> None:
    # Use the fixed runner template (with all 7 audit fixes) as the base
    template_path = Path(__file__).parent / "run-2026-07-20-v2" / "run_on_host_b.py"
    if template_path.exists():
        script = template_path.read_text()
        # Replace the embedded bundle values with placeholders, then substitute
        import re
        script = re.sub(r'BUNDLE_B64 = ".*?"', 'BUNDLE_B64 = "__BUNDLE_B64__"', script, flags=re.DOTALL)
        script = re.sub(r'BUNDLE_SHA256 = ".*?"', 'BUNDLE_SHA256 = "__BUNDLE_SHA256__"', script)
    else:
