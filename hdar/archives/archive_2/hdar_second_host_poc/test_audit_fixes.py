...
# Import the runner as a module (current deploy-package version)
RUNNER_DIR = Path(__file__).parent / "deploy-package"
...
def test_fix7_task_constants():
    ...
    assert hasattr(runner, "TASK_EXPECTED_OUTPUT_HASH"), "runner should have TASK_EXPECTED_OUTPUT_HASH"
    assert len(runner.TASK_EXPECTED_OUTPUT_HASH) == 64, f"TASK_EXPECTED_OUTPUT_HASH should be 64 chars, got {len(runner.TASK_EXPECTED_OUTPUT_HASH)}"
...
def test_fix7_complete_deterministic_task():
    ...
    Uses the actual workspace restored from the capsule."""
    tmpdir = Path(tempfile.mkdtemp(prefix="test-fix7-task-"))
    try:
        workspace = tmpdir / "workspace"
        workspace.mkdir()
        # Extract capsule from tar
        import tarfile as _tar
        capsule_tar = Path(__file__).parent / "_e2b_build" / "transport_capsule_epoch_1_signed.tar.gz"
        capsule_dir = tmpdir / "capsule"
        if capsule_tar.exists():
            with _tar.open(capsule_tar, "r:gz") as tar:
                tar.extractall(capsule_dir)
            # The capsule is extracted as capsule_epoch_1/
            e1_dir = capsule_dir / "capsule_epoch_1"
            # Use the runner's restore_workspace function
            restored = runner.restore_workspace(e1_dir, workspace)
            assert restored.get("exact"), f"workspace restoration should be exact: {restored}"
        else:
            # Fallback: generate workspace using run_e2b_proof.py's build logic
            import importlib.util
            spec = importlib.util.spec_from_file_location("run_e2b_proof", Path(__file__).parent / "run_e2b_proof.py")
            mod = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(mod)
            (workspace / "src").mkdir(parents=True)
            (workspace / "data").mkdir(parents=True)
            mod._create_workspace_files(workspace)
        ...
        assert result["computed_output_hash"] == runner.TASK_EXPECTED_OUTPUT_HASH, \
            f"expected {runner.TASK_EXPECTED_OUTPUT_HASH}, got {result['computed_output_hash']}"
