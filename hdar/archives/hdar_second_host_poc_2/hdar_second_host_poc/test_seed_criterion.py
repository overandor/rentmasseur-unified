...
# Import shared functions from run_on_host_b.py
RUNNER_DIR = Path(__file__).resolve().parent / "run-2026-07-20-v2"
sys.path.insert(0, str(RUNNER_DIR))

# For v2, the runner is in the same dir
from run_on_host_b import (
    ...
    AGENT_ID, SCHEMA, TASK_INPUT_N, TASK_EXPECTED_RESULT,
)
...
def main() -> int:
    ...
    print(f"  Task: {task_result['task']} = {task_result['computed_result']} (expected {task_result['expected_result']})", file=sys.stderr)
