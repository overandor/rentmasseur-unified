// hdar_crypto.cpp — Ed25519 cryptographic kernel using OpenSSL 3.x.
// Native macOS implementation. No Python. No mocks. Pure C++.
...
std::string Ed25519KeyPair::public_key_hex() const { return to_hex(public_key); }
std::string Ed25519KeyPair::private_key_hex() const { return to_hex(private_key); }
...
Ed25519KeyPair Ed25519KeyPair::from_private_hex(const std::string& hex_str) {
    auto raw = from_hex(hex_str);
...
std::string Ed25519PublicKey::hex() const { return to_hex(key); }

Ed25519PublicKey Ed25519PublicKey::from_hex(const std::string& hex_str) {
    auto raw = hdar::from_hex(hex_str);
    Ed25519PublicKey pk;
    pk.key = raw;
    return pk;
}
