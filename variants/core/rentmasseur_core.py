...
CAPTURE_SCREENS = os.getenv("RENTMASSEUR_CAPTURE_SCREENS", "").lower() == "true"
IMPLICIT_WAIT = 10
PAGE_TIMEOUT = 30
DEFAULT_USER_AGENT = (
...
def setup_driver(headless: bool = True) -> webdriver.Chrome:
    ...
    opts.add_argument(f"--user-agent={os.getenv('RENTMASSEUR_USER_AGENT', DEFAULT_USER_AGENT)}")
...
def _capture_screen(driver: webdriver.Chrome, label: str) -> None:
    if not CAPTURE_SCREENS:
        return
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    debug_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "debug")
    os.makedirs(debug_dir, exist_ok=True)
    path = os.path.join(debug_dir, f"screen_{label}_{ts}.png")
    try:
        driver.save_screenshot(path)
        logger.info("Captured screen: %s", path)
    except Exception as e:
        logger.warning("Failed to capture screen %s: %s", label, e)
...
def login(driver: webdriver.Chrome) -> bool:
    ...
    try:
        ...
        time.sleep(5)
        _capture_screen(driver, "login_page")
        page_text = driver.execute_script("return document.body ? document.body.innerText : '';") or ""
        if "CrowdSec" in page_text or "Access Forbidden" in page_text:
            logger.error("RentMasseur blocked this runner before the login form loaded")
            _dump_debug(driver, "login_access_forbidden")
            return False
        for xpath in [
        ...
        if LOGIN_URL not in driver.current_url:
            ...
            _capture_screen(driver, "login_success")
            return True
...
def set_availability_24_7(driver: webdriver.Chrome) -> bool:
    try:
        ...
        time.sleep(3)
        _capture_screen(driver, "availability_before")
        ...
        driver.get(AVAILABILITY_URL)
        time.sleep(2)
        _capture_screen(driver, "availability_after_save")
        verified = driver.execute_script("""
            ...
            if (!statusSelect) return {ok: false, error: 'no_status_select_after_save'};
            const statusText = statusSelect.options[statusSelect.selectedIndex]?.text || '';
            const status = statusText.toLowerCase();
            return {
                ok: status.includes('available') && !status.includes('not'),
                status: statusText
            };
        """)
        if not isinstance(verified, dict) or not verified.get("ok"):
            logger.error("Availability verification failed: %s", verified)
            _dump_debug(driver, "availability_verify_failed")
            return False
        logger.info("Availability verified: status=%s", verified.get("status"))
        return True
    except Exception as e:
