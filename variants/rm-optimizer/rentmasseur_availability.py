...
def set_availability_24_7(driver: webdriver.Chrome) -> bool:
    ...
    After setting, reads back the actual availability status and expiry time
    from the page to verify the set was applied. Logs the confirmed expiry
    and the login timestamp so CI logs show whether the reset actually took.
    """
    ...
    try:
        ...
        ok = driver.execute_script("""
            ...
            if (timeSelect) {
                ...
                if (durationOpts.length > 0) {
                    ...
                    selectedDuration = longest.text.trim();
                }
            }
            ...
            return {ok: true, duration: selectedDuration};
        """)
        ...
        selected_duration = ok.get('duration', 'unknown') if isinstance(ok, dict) else 'unknown'
        logger.info("Availability set via JS automation (duration: %s)", selected_duration)
        time.sleep(3)  # Wait for the set to take effect

        # --- Post-set verification: read back the actual state from the page ---
        # Reload the page to get the current confirmed state, not the form
        # we just submitted.
        driver.get(AVAILABILITY_URL)
        time.sleep(3)
        dismiss_popups(driver)
        ...
        verified = driver.execute_script("""
            // Read back the page text to find the current availability status
            // and any expiry/timeout information.
            const body = document.body ? document.body.innerText : '';
            const selects = Array.from(document.querySelectorAll('select'));

            // Check what the status select currently shows
            const statusSelect = selects.find(s => {
                const opts = Array.from(s.options).map(o => o.text.toLowerCase());
                return opts.includes('available') || opts.includes('not set');
            });
            ...
            if (statusSelect && statusSelect.selectedIndex >= 0) {
                currentStatus = statusSelect.options[statusSelect.selectedIndex].text.trim();
            }

            // Look for any text mentioning expiry, timeout, expires, until, etc.
            const expiryMatch = body.match(/(?:expir|timeout|until|valid|active)(?:es|ed)?[:\\s]+([^\\n]+)/i);
            const expiryText = expiryMatch ? expiryMatch[1].trim() : '';

            // Look for a time pattern like "6 hours" or "360 minutes"
            const durationMatch = body.match(/(\\d+)\\s*(hour|minute|min|hr)/i);
        ...
        if verified:
            current_status = verified.get('status', 'unknown')
            expiry_text = verified.get('expiry_text', '')
            duration_text = verified.get('duration_text', '')
            logger.info("Post-set verification — status: %s, duration: %s, expiry: %s",
                        current_status, duration_text or selected_duration, expiry_text or 'not found')
            logger.info("Login timestamp (UTC): %s", login_timestamp.isoformat())

            # Verify the status is actually 'Available' (not 'Not Set' or 'Not Available')
            if 'available' not in current_status.lower():
                logger.error("Post-set verification FAILED — status is '%s', expected 'Available'", current_status)
                _dump_debug(driver, "availability_verification_failed")
                return False

            logger.info("Post-set verification PASSED — availability confirmed as '%s'", current_status)
        else:
            logger.warning("Post-set verification could not read page state — assuming success (no verification)")
...
def run_once(headless: bool = True) -> bool:
    ...
    try:
        ...
        login_start = datetime.now(timezone.utc)
        if not login(driver):
            logger.error("Login failed at %s — availability NOT set", login_start.isoformat())
            return False
        login_end = datetime.now(timezone.utc)
        logger.info("Login confirmed at %s (took %.1fs)",
                    login_end.isoformat(), (login_end - login_start).total_seconds())
