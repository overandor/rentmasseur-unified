...
def ui():
    dashboard = APP_DIR / "static" / "dashboard.html"
    if dashboard.exists():
...
if STATIC_DIR.exists():
    app.mount("/static", _StaticFiles(directory=str(STATIC_DIR)), name="static_files")


def _files_conn():
    conn = _sqlite3.connect(str(FILES_DB))
...
def files_ui():
    ...
    return JSONResponse({"status": "GRAY_NO_DATA", "message": "No static UI found"})


@app.get("/api/files/stats")
def files_stats():
    if not FILES_DB.exists():
        return {"status": "GRAY_NO_DATA", "total_files": 0, "message": "No index DB. Run index_files.py first."}
    conn = _files_conn()
    total = conn.execute("SELECT COUNT(*) FROM files").fetchone()[0]
    size_sum = conn.execute("SELECT COALESCE(SUM(size), 0) FROM files").fetchone()[0]
    exts = conn.execute(
        "SELECT extension, COUNT(*) as c FROM files GROUP BY extension ORDER BY c DESC LIMIT 20"
    ).fetchall()
    last_run = conn.execute("SELECT * FROM indexing_runs ORDER BY id DESC LIMIT 1").fetchone()
    conn.close()
    return {
        "status": "GREEN_REAL" if total > 0 else "GRAY_NO_DATA",
        "total_files": total,
        "total_bytes": size_sum,
        "extensions": [{"ext": r[0] or "(none)", "count": r[1]} for r in exts],
        "last_run": dict(last_run) if last_run else None,
    }


@app.get("/api/files/search")
def files_search(q: str = "", ext: str = "", limit: int = 100, offset: int = 0):
    if not FILES_DB.exists():
        return {"files": [], "status": "GRAY_NO_DATA"}
    conn = _files_conn()
    params = []
    where = []
    if q:
        where.append("(path LIKE ? OR dirname LIKE ?)")
...
def files_duplicates(limit: int = 50):
    if not FILES_DB.exists():
        return {"groups": [], "status": "GRAY_NO_DATA"}
    conn = _files_conn()
    rows = conn.execute(
        "SELECT * FROM files WHERE sha256 IN (SELECT sha256 FROM files GROUP BY sha256 HAVING COUNT(*) > 1) ORDER BY sha256, size DESC LIMIT ?",
        (limit,),
    ).fetchall()
    conn.close()
    groups = {}
    ...
    return {"groups": [{"sha256": k, "files": v} for k, v in groups.items()]}
