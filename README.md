...
This repo combines all RentMasseur project files from multiple locations on this machine into one repo.

### Top-level (main extension + control plane)
```
/
  Dockerfile
  README.md
  ...
  rentmasseur_core.py        — core logic
  rentmasseur_optimizer.py   — optimizer
  rentmasseur_coordinator.py — coordinator
  rentmasseur_availability.py — availability manager
  rm_engagement_engine.py    — engagement engine
  rm_bio_loop.py             — bio A/B loop
  rm_demo_agent.py           — demo agent
  rm_selenium_cicd.py        — Selenium CI/CD
  rm_proof.py                — proof system
  rm_telemetry_poller.py     — telemetry
  cartman_engine.py          — cartman engine
  cartman_watcher.py         — cartman watcher
  server.py                  — Flask API server
  hf_app.py                  — HuggingFace app
  content/                   — bios, blog posts, metrics
  receipts/                  — tamper-evident receipt chain
  rm_automation/             — automation framework
  rm_ci/                     — CI/CD pipeline
  rm_traffic/                — traffic module
  hdar/                      — HDAR dashboards
  vercel/                    — Vercel deployment
  deploy/launchagents/       — macOS LaunchAgents
  docs/                      — documentation
```

### Variants (merged from other locations)
```
variants/
  rm-optimizer/    — CascadeProjects/rm-optimizer (full optimizer + server + dashboard)
  visitor/         — CascadeProjects/rentmasseur-visitor (Next.js visitor frontend)
  daemon/          — Downloads/rentmasseur-optimizer (daemon variant with systemd)
  core/            — ~/rentmasseur-optimizer (minimal core with strategies)
  scraper/         — rentmasseur_scraper (Rust+Python scraper, bio analysis, dashboards)
  bi/              — HyperFlow BI module (rentmasseur_bi.py + data)
  rentmasseur-infra-combined.py — combined infra script
```
